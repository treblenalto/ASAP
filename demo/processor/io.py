...
class IO():
    ...
    def init_environment(self): #torchlight.torchlight.io.IO
        ...
        # gpu
        if self.arg.use_gpu: #gpus = torchlight.torchlight.gpu.
            gpus = torchlight.gpu.visible_gpu(self.arg.device)
            torchlight.gpu.occupy_gpu(gpus)
            self.gpus = gpus
            self.dev = "cuda:0"
        else:
            self.dev = "cpu"
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('-c', '--config', default='./work_dir/stgcn_demo.yaml', help='path to the configuration file')
